...
def play(env, runner, train_cfg, wandb_run):
    # * set up recording
    ...
    for i in range(10 * int(env.max_episode_length)):
        ...
        for i in range(env.num_envs):
            logging_dict[f"Error/z_position_env_{i}"] = (
                env.hl_commands[i, 2] - env.base_pos[i, 2]
            )
            logging_dict[f"Error/z_velocity_env_{i}"] = (
                env.hl_commands[i, -1] - env.base_lin_vel[i, -1]
            )
        env.check_exit()
...
if __name__ == "__main__":
    ...
    args = get_args()
    run = wandb.init(
        project="Bouncing Ball LL Graphs",
        entity="biomimetics",
        name="Height Error in Constant Resample Play",
    )
    with torch.no_grad():
        env, runner, train_cfg = setup(args)
        play(env, runner, train_cfg, run)
