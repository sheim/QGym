...
class SliderInterface:
    def __init__(self, env):
        self.master = Tk()
        self.w1 = Scale(self.master, from_=-50, to=50, length=500,tickinterval=10, orient=HORIZONTAL)
        self.w2 = Scale(self.master, from_=-50, to=50, length=500,tickinterval=10, orient=HORIZONTAL)
        self.w3 = Scale(self.master, from_=-50, to=50, length=500,tickinterval=10, orient=HORIZONTAL)
    ...
    def update(self, env):
        ...
        if mode == 'one_leg':
            ...
            eigenvectors = np.array([[-0.02939241,  0.81596737,  0.57735027],
            ...
            for s in range(len(scalings)):
                ...
                env.dof_pos_target[:,9:12] += torch.from_numpy(scalings[s] * eigenvector).to(torch.float).to(device='cuda') 
    
        elif mode == 'joint':
            #3rd actuator kfe, all legs
            eigenvectors = np.array([[ 0.30653829, -0.76561209, -0.26433388],
            ...
            for s in range(len(scalings)):
                ...
                env.dof_pos_target[:,:] += torch.from_numpy(scalings[s] * fullvec).to(torch.float).to(device='cuda') 
        
        elif mode =='all':
            #all legs all actuators
            eigenvectors = np.array([[-0.03376138, -0.02442507,  0.03189318],
            ...
            for s in range(len(scalings)):
                ...
                env.dof_pos_target[:,:] += torch.from_numpy(scalings[s] * eigenvector).to(torch.float).to(device='cuda') 
