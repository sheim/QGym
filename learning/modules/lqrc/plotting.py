...
def plot_pendulum_critics_with_data(
    x,
    predictions,
    targets,
    actions,
    ...
    action_cmap = mpl.cm.get_cmap("bwr")
    ...
    for idx, critic_name in enumerate(x):
        np_predictions = predictions[critic_name].detach().cpu().numpy().reshape(-1)
        np_targets = (
            targets["Ground Truth MC Returns"].detach().cpu().numpy().reshape(-1)
        )
        ...
        axes[0, idx].set_title(f"{critic_name} Prediction")
    ...
    # Plot Actions
    np_actions = actions.detach().cpu().numpy().reshape(-1)
    axes[1, 1].imshow(
