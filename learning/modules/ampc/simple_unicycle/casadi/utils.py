...
def plot_3d_costs(
    ...
    is_nested = isinstance(
        xy_coords[0][0], list
    )  # Assumes that nested lists indicate a list of lists
    ...
    if is_nested:
        ...
        for idx, (outer_x0_set, cost_set, color) in enumerate(
            zip(xy_coords, costs, colors)
        ):
        ...
        if linLatW is not None and linLatb is not None:
            Z = np.array(
                [
                    [
                        (
                            (np.array([xi, yi, *zero_padding]) - offset) @ linLatW.T
                            + linLatb
                        )
                        @ P
                        @ (
            ...
            Z = np.array(
                [
                    [
                        (np.array([xi, yi, *zero_padding]) - offset)
                        @ P
                        @ (np.array([xi, yi, *zero_padding]) - offset)
                        if mask[i, j]
                        else np.nan
    ...
    if cost_gradient is not None:
        ...
        if is_nested:
            for x0_set, costs_, cost_gradient_, color in zip(
                xy_coords, costs, cost_gradient, colors
            ):
...
def plot_learning_progress(
    ...
    for name, value in test_error.items():
        ax1.plot(np.array(value), label=name)
