...
# make paths for graph
save_path = f"{LEGGED_GYM_ROOT_DIR}/learning/modules/ampc/simple_unicycle/casadi"
...
def f(x_, u_):
    ...
    # a_, dtheta_ = u_
    return vertcat(x_[2] * cos(x_[3]), x_[2] * sin(x_[3]), u_[0], u_[1])
...
def make_run_fcn(
    ...
    for (xobs, yobs), r in zip(obstacle_positions, obstacle_radiuses):
        for k in range(N + 1):
            opti.subject_to(
                (px[k] - xobs) ** 2 + (py[k] - yobs) ** 2
                >= (r + inflation - eps_obst[k]) ** 2
            )
    for i in range(eps.shape[0]):
        cost += (
            50 * soft_constraint_scale * eps[i] ** 2 + soft_constraint_scale * eps[i]
        )
    for i in range(eps_obst.shape[0]):
        cost += (
            50 * soft_constraint_scale * eps_obst[i] ** 2
            + soft_constraint_scale * eps_obst[i]
        )
    # ---- boundary conditions --------
    ...
    if silent:
        opti.solver(
            ...
            # | options, # ! TODO: return to issue of Python 3.8 if this becomes necessary
        )
    else:
    ...
    def run(x0):
        ...
        try:
            ...
            plt_name = None
            if plt_save:
                plt_name = f"{save_path}/graphs/open_loop_ipopt_x0=[{x0[0]:.2f},{x0[1]:.2f}].pdf"
...
def evaluate_grid_closed_loop(Ngrid=21, filename="4d_data"):
    print(f"Ngrid = {Ngrid}")
        ...
        for i in range(Ngrid):
            for j in range(Ngrid):
                for k in range(Ngrid):
                    ...
                    pbar.set_postfix({"cost": costtraj[0]})  # Update with last cost
                    pbar.update(1)  # Update the progress bar for each iteration
...
def solve_closed_loop(xinit, noise=[0.005, 0.005, 0.005, 0.5], plt_show=None):
    ...
    if plt_show:
        plot_robot(
            ...
            time_label="time [s]",
            plt_show=True,
            plt_name=f"{save_path}/graphs/closed_loop_ipopt_x0=[{xinit[0]:.2f},{xinit[1]:.2f}].pdf",
