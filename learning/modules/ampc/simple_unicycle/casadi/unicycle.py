    ...
    with tqdm.tqdm(total=Ngrid * Ngrid) as pbar:
        for i in range(Ngrid):
            for j in range(Ngrid):
                x0 = [X[i, j], Y[i, j]]
                Utraj, Xtraj, cost = solve_open_loop(x0, soft_constraint_scale=1e3, soft_state_constr=True, silent=True, plt_show=False)
                if cost is not None:
                    x0s.append(x0)
                    Utrajs.append(Utraj)
                    Xtrajs.append(Xtraj)
                    costs.append(cost)
                pbar.set_postfix({"last_cost": cost if cost is not None else 'N/A'})  # Update with last cost
