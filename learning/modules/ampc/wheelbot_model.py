...
def export_wheelbot_ode_model():
    ...
    f_impl_q = cs.vertcat(psi_dot, phi_dot, theta_dot, q_W_dot, q_R_dot) - cs.vertcat(
        dpsi, dphi, dtheta, omega_W, omega_R
    )
    f_impl_dq = (
        M @ cs.vertcat(dpsi_dot, dphi_dot, dtheta_dot, omega_W_dot, omega_R_dot)
        + b
        + g
        + tau_contact
    )
