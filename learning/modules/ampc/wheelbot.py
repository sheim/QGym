...
import casadi as cs
from acados_template import AcadosSim, AcadosSimSolver, AcadosSimBatchSolver, AcadosOcp, AcadosOcpSolver
...
class WheelbotBatchSimulation:
    def __init__(self, num_threads_in_batch_solve, batch_size, delta_parameter_values=np.zeros(11)):
        ...
        self.batch_integrator  = AcadosSimBatchSolver(self.sim, batch_size, verbose=False)
...
def plot_wheelbot(data_x, data_u, plot_labels, subplot_labels_x = default_subplot_labels_x, subplot_labels_u = default_subplot_labels_u, filename="wheelbot_plot", show=True):
    # Plot each column
    nx = data_x[0].shape[0]
    nu = data_u[0].shape[0]
    ...
    x0 = list(data_x[0][:,0])
    x0_formatted_floats = "[" + ", ".join(["{:.2f}".format(number) for number in x0]) + "]"
...
class WheelbotOneStepMPC():
    ...
    def reset(self):
        self.acados_ocp_solver = AcadosOcpSolver(self.ocp, json_file = self.solver_json, generate = False, build = False)

    def run(self, x0, A):
        # print(self.acados_ocp_solver.get_option)
        self.acados_ocp_solver.cost_set(1, "W", A)
