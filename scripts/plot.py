...
def create_plot(phases, variable_name, data, keys):
    ...
    for k in range(0, 4):
        ...
        plt.savefig(f"figures/plot_{variable_name}_last5_{k}")
        handles = []
        plt.cla()
...
def create_plot_small(phases, variable_name, data, keys):
    ...
    for k in range(0, 4):
        ...
        plt.xlabel("Phase (rad)", fontsize=20)
        plt.title(f"{variable_name}_{k} vs Phase", fontsize=20)
        plt.savefig(f"figures/plot_{variable_name}_last5_{k}")
        plt.cla()


def plot_data(variable_name, full_data, df_phase):
    data, smalldata, phases, new_vardata, samples = process_data(
        variable_name, full_data, df_phase
    )
...
def process_data(variable_name, full_data, df_phase):
    ...
    keys = full_data[
        "dof_names"
    ]  # for humanoid: ["right_hip_yaw", "right_hip_abad","right_hip_pitch","right_knee","right_ankle","left_hip_yaw","left_hip_abad","left_hip_pitch","left_knee","left_ankle"]
    ...
    if new_vardata.shape[0] / samples > 4:
        ...
        for i in range(0, new_vardata.shape[0], m):
            for k in range(4):
                smalldata[str(k + 1)] = np.vstack(
                    [smalldata[str(k + 1)], new_vardata[i + k, :]]
                )
...
def phase_plot(full_data, df_phase):
    ...
    # plot
    handles = []
    fig, (ax1, ax2, ax3, ax4) = plt.subplots(1, 4, figsize=(8 * 4, 8))
    ...
    for i in range(0, 4):
        plotted_joint = names[
            i * 3 + 2
        ]  # Change index to change which actuator type is plotted
        color_map = ["r", "g", "b"]
        for k in range(1, len(command_indexes)):
            color = color_map.pop()
            for j in range(45, 50):
                (line,) = axes[i].plot(
                    q[plotted_joint][command_indexes[k - 1] : command_indexes[k], j],
                    qd[plotted_joint][command_indexes[k - 1] : command_indexes[k], j],
                    "-o",
                    markersize=2,
                    color=color,
                    label=command_indexes[k],
                )
            handles.append(line)
        ...
        # plt.show()
        handles = []
    fig.savefig(f"figures/Phase Plot")
...
def time_plot(full_data, df_phase):
    ...
    for k in range(0, 4):
        ...
        for i in range(k * 3, k * 3 + 3):
            ...
            for j in range(45, 50):
                (line,) = plt.plot(
                    q[keys[i]][:, j], "-o", markersize=2, color=color, label=keys[i]
                )
            handles.append(line)
        ...
        plt.legend(handles=handles)
        # plt.show()
        plt.savefig(f"figures/Time Plot_leg{k}")
        handles = []
        plt.cla()
...
def check_means(variable_name, full_data, df_phase):
    ...
    for i in range(0, 4):
        for j in range(0, 4):
            means_comp[i, j] = np.allclose(
                means[i * 3 : i * 3 + 3], means[j * 3 : j * 3 + 3], atol=1e-1
            )
            std_comp[i, j] = np.allclose(
                std[i * 3 : i * 3 + 3], std[j * 3 : j * 3 + 3], atol=0.6
            )
    print(keys)
    print(means_comp)
    print(std_comp)
...
def reorg_plot_data(variable_name, full_data):
    ...
    if m > 1:
        ...
        a = Arrow3D(
            [0, 1], [0, 0], [0, 0], mutation_scale=5, lw=1, arrowstyle="-|>", color="b"
        )
        ax.add_artist(a)
        a = Arrow3D(
            [0, 0], [0, 1], [0, 0], mutation_scale=5, lw=1, arrowstyle="-|>", color="b"
        )
        ax.add_artist(a)
        a = Arrow3D(
            [0, 0], [0, 0], [0, 1], mutation_scale=5, lw=1, arrowstyle="-|>", color="b"
        )
        ax.add_artist(a)
        ax.set_xlabel("$X$")
        ax.set_ylabel("$Y$")
        ...
        plt.savefig(f"figures/plot_{variable_name}")
    else:
        # plt.show()
        fig = plt.figure(figsize=(10, 10))
