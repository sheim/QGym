...
def play(env, runner, train_cfg, init_conditions):
    ...
    env.dof_state[:, 0] = torch.from_numpy(
        np.hstack((init_conditions[0, 0], init_conditions[0, 2]))
    ).to(DEVICE)
    env.dof_state[:, 1] = torch.from_numpy(
        np.hstack((init_conditions[0, 1], init_conditions[0, 3]))
    ).to(DEVICE)
