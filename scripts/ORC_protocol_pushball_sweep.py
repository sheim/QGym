...
def get_run_names(experiment_name):
    experiment_path = os.path.join(LEGGED_GYM_ROOT_DIR, "logs", experiment_name)
    return [
        folder_name
...
def create_logging_dict(runner, test_total_timesteps):
    ...
    for state in states_to_log:
        ...
        states_to_log_dict[state] = torch.zeros(
            (runner.env.num_envs, test_total_timesteps, array_dim),
...
def setup(toggle, run_name):
    ...
    env_cfg.env.episode_length_s = 100.0
    env_cfg.init_state.timeout_reset_ratio = 1.0
    env_cfg.domain_rand.randomize_base_mass = False
    env_cfg.domain_rand.randomize_friction = False
    env_cfg.terrain.mesh_type = "plane"
    env_cfg.osc.init_to = "random"
    env_cfg.osc.init_w_offset = False
    env_cfg.osc.process_noise = 0.0
    env_cfg.osc.omega_var = 0.0
    env_cfg.osc.coupling_var = 0.0
    env_cfg.commands.ranges.lin_vel_x = [0.0, 0.0]
    env_cfg.commands.ranges.lin_vel_y = 0.0
    env_cfg.commands.ranges.yaw_vel = 0.0
    env_cfg.commands.var = 0.0
    env_cfg.env.env_spacing = 3.0
    ...
    env_cfg, train_cfg = adjust_settings(
        toggle=toggle, env_cfg=env_cfg, train_cfg=train_cfg
    )
...
def play(env, runner, train_cfg, log_file_path, run_name):
    push_mags = torch.arange(0.5, 4.0, 0.5)
    ...
    for push_mag in push_mags:
        ...
        assert (
            n_trials == env.num_envs
        ), f"number of trials: {n_trials}, number of loaded environments: {env.num_envs}"
        env.commands[:, 0] = 3.0
        ...
        states_to_log_dict = run_protocol(
            env, runner, push_ball, num_staggers, stagger_timesteps, n_directions
        )

        # * save data
        states_to_log_dict_cpu = {
            k: v.detach().cpu().numpy() for k, v in states_to_log_dict.items()
        }

        save_path = os.path.join(log_file_path, f"{push_mag:.1f}")
        os.makedirs(save_path, exist_ok=True)
        save_name = os.path.join(save_path, f"{run_name}_data.npz")
        np.savez_compressed(save_name, **states_to_log_dict_cpu)
        env.reset()


def run_protocol(env, runner, push_ball, num_staggers, stagger_timesteps, n_directions):
    kdx = 0  # trial counter
    entrainment_time = 5.0

    test_start_time = 1.0
    test_total_time = 9.0 + test_start_time
    test_total_timesteps = int(test_total_time / env.cfg.control.ctrl_dt)

    states_to_log, states_to_log_dict = create_logging_dict(
        runner, test_total_timesteps
    )
    ...
    for t in range(int(test_total_time / env.cfg.control.ctrl_dt)):
        ...
        if (
            t
            == int(test_start_time / env.cfg.control.ctrl_dt) + stagger_timesteps * kdx
        ):
...
def worker(toggle, run_name):
    with torch.no_grad():
        ...
        log_file_path = os.path.join(
            LEGGED_GYM_ROOT_DIR, "gym", "scripts", "FS_data", train_cfg.runner.run_name
        )
