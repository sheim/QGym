...
plotting_options = {
    ...
    "100": {"color": "yellowgreen", "linestyle": "solid", "linewidth": 3},
    "110": {"color": "goldenrod", "linestyle": (0, (3, 1, 1, 1, 1, 1)), "linewidth": 2},
    "111": {"color": "crimson", "linestyle": "solid", "linewidth": 3},
}
    ...
    for run_name in run_names:
        ...
        with np.load(data_path, allow_pickle=True) as loaded_data:
            if len(obs2use) == 1:
                ...
                data = np.concatenate(
                    list(loaded_data[name] for name in obs2use), axis=2
                )
...
custom_bin_edges = [
    np.linspace(global_min[i], global_max[i], num_bins[i] + 1)
...
fig.text(
    ...
    ha="left",
    va="top",
    bbox={"facecolor": "white", "edgecolor": "grey", "boxstyle": "round, pad=0.5"},
)  # , fontsize=10)
