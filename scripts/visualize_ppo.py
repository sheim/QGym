...
for it in IT_RANGE:
    # load data
    base_data = torch.load(os.path.join(RUN_DIR, "data_{}.pt".format(it))).to(DEVICE)
    ...
    with torch.no_grad():
        ...
        graphing_data["values"]["V-Critic"] = critic.evaluate(data[0, :]["critic_obs"])
        graphing_data["returns"]["V-Critic"] = data[0, :]["returns"]
        graphing_data["actions"]["V-Critic"] = actor(data[0, :]["actor_obs"])
