...
def get_counts(data, bin_edges, method="digitize"):
    M, D = data.shape
    if method == "digitize":
        digi_data = [np.digitize(data[:, i], bins=bin_edges[i]) for i in range(D)]
...
def sliding_window_entropy_single_trial(
    data, window_size, step_size, method, bin_edges
):
    M, D = data.shape
    entropy_values = []
...
if __name__ == "__main__":
    data_path = (
        "/home/heim/Repos/trigym/gym/scripts/osc_data/ORC_111/noise_step_1.0.npz"
    )
    # dimensions = [0, 1, 2]
    window_size = 72
    step_size = 1
    ...
    custom_bin_edges = [
        np.linspace(np.min(data[:, :, i]), np.max(data[:, :, i]), num_bins[i] + 1)
    ...
    if plot_trials:
        ...
        z_value = (
            1.96
        )  # z-value for 95% confidence (from the standard normal distribution)
