...
def eval(log, vel, env, run_name, foot_pos):
    ...
    log["base_lin_vel"][np.logical_and(log["base_lin_vel"]>=-1e-1, log["base_lin_vel"]<=1e-1)] = 0 
    log["base_ang_vel"][np.logical_and(log["base_ang_vel"]>=-3e-1, log["base_ang_vel"]<=3e-1)] = 0
    ...
    #norm
    vel_mag = np.array([np.mean((log["base_lin_vel"][:,:1] - np.ones_like(log["base_lin_vel"][:,:1])*vel[0])**2)])
    ...
    start_groups = plotgrf(run_name, log, vel, env.cfg.control.ctrl_frequency, show=True)
    ...
    return {'power': power, 'vel_dir': vel_dir, 'vel_mag': vel_mag,
            'termination': termination, 'deltas': deltas}
                ...
                try:
                    ...
                    for i, y in enumerate(yaw):
                        for j, x in enumerate(forward_backward):
                            ...
                            index = np.where((velocity_sweep[:,0] == x) & (velocity_sweep[:,1] == y))
                            if index[0].size > 0:
                                success_rate_grid[i, j] = successes[key][index[0]]

                    # Create the heatmap
                    plt.imshow(success_rate_grid, aspect='auto', cmap='viridis', origin='lower',
                            extent=[yaw.min(), yaw.max(), forward_backward.min(), forward_backward.max()], 
                            vmin=scalings[key][0], vmax=scalings[key][1])
                    ...
                    file_path = os.path.join('/home/aileen/QGym/scripts/gridplots/'+run_name)
                    ...
                    print(f"Failed to plot heatmap for key {key} due to runtime warning: {e}")
...
def plotgrf(run_name, log, vel, sim_freq, show=False):
    ...
    if vel[0] !=0 and np.sum(zeros) >0:
        start = log['grf']*np.vstack((np.zeros((1,log['grf'].shape[1],log['grf'].shape[2])), zeros[0:-1,:,:]))>0
        end = log['grf']*np.vstack((zeros[1:,:,:], np.zeros((1,log['grf'].shape[1],log['grf'].shape[2]))))>0

        start = np.vstack((start[1:,:,:],np.zeros((1,log['grf'].shape[1],log['grf'].shape[2]))))
        end = np.vstack((np.zeros((1,log['grf'].shape[1],log['grf'].shape[2])),end[:-1,:,:]))
        ...
        start_indices = np.array(list(zip(start_row_indices, start_col_indices, foot_start)))
        ...
        start_indices_sorted = start_indices[np.lexsort((start_indices[:, 0], start_indices[:, 1], start_indices[:, 2]))]
        end_indices_sorted = end_indices[np.lexsort((end_indices[:, 0], end_indices[:, 1], end_indices[:, 2]))]
        ...
        for foot_index in range(4):
            ...
            start_groups[foot_index] = {col: [] for col in np.unique(start_indices_sorted[start_indices_sorted[:, 2] == foot_index, 1])}
            end_groups[foot_index] = {col: [] for col in np.unique(end_indices_sorted[end_indices_sorted[:, 2] == foot_index, 1])}
            ...
            for col in sorted(start_groups[foot_index].keys()):
                ...
                if end_groups[foot_index][col] and end_groups[foot_index][col][0] < start_groups[foot_index][col][0]:
                    end_groups[foot_index][col] = end_groups[foot_index][col][1:]

                # Ensure both groups have the same length
                min_length = min(len(start_groups[foot_index][col]), len(end_groups[foot_index][col]))
                start_groups[foot_index][col] = start_groups[foot_index][col][:min_length]
                end_groups[foot_index][col] = end_groups[foot_index][col][:min_length]
                diff = np.max(np.array(end_groups[foot_index][col])-np.array(start_groups[foot_index][col]))
                if diff > maxlen:
                    maxlen = diff
        for foot_index in range(4):
            all_interpolated_segments = []
            for col in sorted(start_groups[foot_index].keys()):
                for start_row, end_row in zip(start_groups[foot_index][col], end_groups[foot_index][col]):
                    if start_row < end_row:  # Ensure valid segment
                        segment = np.append(log['grf'][start_row:end_row, col, foot_index],np.zeros((150-(end_row-start_row),))) #42
...
def phase_plot(successes):
    ...
    ax.scatter(successes['deltas'][:,0], successes['deltas'][:,1], successes['deltas'][:,2],alpha=0.5)
...
def plot_foot_dof(foot_pos, grf, run_name, vel):
    ...
    for i, key in enumerate(foot_pos.keys()):
        ...
        print(np.tile(np.mean(foot_pos[key][0,:,:], axis = 0), (foot_pos[key][0,:,:].shape[0],1)).shape)
        foot_pos_norm = foot_pos[key][0,:,:] - np.tile(np.mean(foot_pos[key][0,:,:], axis = 0), (foot_pos[key][0,:,:].shape[0],1))
        proj_data, eigvecs, var = sklearnpca(foot_pos_norm)
        print(foot_pos_norm.shape)
        print(eigvecs)
        print(proj_data.shape)
        ax3d.plot3D(foot_pos_norm[:,0], foot_pos_norm[:,1], foot_pos_norm[:,2], label=key, c = colors[i])
        ax3d.scatter3D(foot_pos_norm[:,0]*grf, foot_pos_norm[:,1]*grf, foot_pos_norm[:,2]*grf, s=5, alpha=0.2, c = 'black', zorder=2)
        ax3d.plot3D([0,eigvecs[0,0]*0.1], [0,eigvecs[1,0]*0.1], [0,eigvecs[2,0]*0.1], label=key, c = colors[i])
        ax3d.plot3D([0,eigvecs[0,1]*0.1], [0,eigvecs[1,1]*0.1], [0,eigvecs[2,1]*0.1], label=key, c = colors[i])
        ax.scatter(proj_data[0,:]*grf,proj_data[1,:]*grf, s=5, alpha=0.2, c = 'black', zorder=2)
