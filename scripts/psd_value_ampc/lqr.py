...
from train import PDCholesky
from learning.modules.ampc.simple_unicycle.casadi.utils import plot_robot
...
def eval_lqr(filename, xinit):
    ...
    with open(f"{LEGGED_GYM_ROOT_DIR}/models/normalization.pkl", "rb") as f:
        data = pickle.load(f)
        V_max = data["cost_max"]
        V_min = data["cost_min"]
        X_max = data["x0_max"]
    ...
    model = PDCholesky(input_dim=len(X_max))
    model.load_state_dict(
        torch.load(f"models/{filename}_{type(model).__name__}.pth", map_location="cpu")
    )
    model.eval()

    def eval_model(x):
        x_normalized = 2 * (x - X_min) / (X_max - X_min) - 1
        _, A_normalized_, x_off_normalized_ = model(
...
if __name__ == "__main__":
    ...
    for x in x0s:
        eval_lqr(filename="4d_data_9261", xinit=x)
