...
for iteration in [50, 100, 150, 200]:
    ...
    # load critic
    model = torch.load(os.path.join(log_dir, "model_{}.pt".format(iteration)))
    ...
    plot_pendulum_multiple_critics_w_data(
        ...
        fn=plot_dir + f"/{name}_CRITIC_it{iteration}",
        data=graphing_obs[:num_steps, traj_idx],
    )

    plt.close()

this_file = os.path.join(LEGGED_GYM_ROOT_DIR, "scripts", "visualize_value_function.py")
