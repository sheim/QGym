def adjust_settings(toggle, env_cfg, train_cfg):
    ...
    if train_cfg.runner.experiment_name == "":
        ...
        train_cfg.runner.experiment_name = (
            "ORC_" + toggle + "_" + train_cfg.runner.experiment_name
        )
    ...
    if toggle[1] == "0":
        # * non reward
        train_cfg.policy.reward.weights.swing_grf = 0.0
        train_cfg.policy.reward.weights.stance_grf = 0.0
    if toggle[2] == "0":
        # * no coupling
        env_cfg.osc.coupling = 0.0
        env_cfg.osc.coupling_range = [0.0, 0.0]
        coupling_stop = 0.0
        coupling_step = 0.0
        coupling_slope = 0.0
        env_cfg.osc.coupling_max = 0.0
