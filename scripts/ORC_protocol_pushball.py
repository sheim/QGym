...
def play(env, runner, train_cfg, png_folder):
    ...
    for t in range(int(test_total_time / env.cfg.control.ctrl_dt)):
        ...
        if RECORD_VID and t % 5:
            filename = os.path.join(png_folder, f"{img_idx}.png")
...
def save_gif(png_folder, gif_folder, frame_rate):
    png_files = sorted(glob.glob(f"{png_folder}/*.png"), key=os.path.getmtime)
...
def worker(toggle, run_name):
    with torch.no_grad():
        ...
        states_to_log_dict = play(
            env, runner=runner, train_cfg=train_cfg, png_folder=png_folder
        )
